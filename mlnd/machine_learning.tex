...
Maximizing the probability of the arrangement would in turn minimize the error. However, there is a problem with the approach for calculating the probability of the arrangement. In the case of thousands of data points, the product of probabilities between 0 and 1 becomes tiny. Therefore, we need a similar function that expresses the probability of the arrangement but with summation instead. This function is the logarithmic function, since it turns products into sums: $log(ab) = log(a) + log(b)$. So the probability of the arrangement becomes:

\begin{equation}
  CE = - \sum_{i = 1}^{N} y_i ln(P_i) + (1 - y_i) ln(1 - P_i)
\end{equation}

Where $y$ is the label (1 or 0). This expression is called cross entropy. The lower the cross entropy, the better the model. The negative of the logarithm of the probability for each point $(-ln(P_i))$ also can be interpreted as error for the corresponding point. This changes our goal from maximizing probability to minimizing cross entropy. Cross entropy is the connection between probability and error. For multilabel cross entropy, the expression is:

\begin{equation}
  CE = - \sum_{i = 1}^{n} \sum_{j = 1}^{m} y_{ij} ln(P_{ij})
\end{equation}

Where $m$ is the number of classes. Our final error function will depend on the cross entropy for each label. For a two label problem, the error function looks like this:

\begin{equation}
  E(W, b) = - \frac{1}{m} \sum_{i = 1}^{m} (1 - y_i) ln(1 - \sigma(Wx^{(i)} + b)) + y_i ln(\sigma(Wx^{(i)} + b))
\end{equation}

The multilabel error formula can be derived from the multilabel cross entropy formula.

\subsubsection{Gradient Descent}
The pseudocode algorithm for gradient descent is:

\begin{enumerate}
  \item Start with random weights W and bias b and draw a line based on them.
  \item Calculate the error for every point.
  \item For every point x:
  \begin{itemize}
    \item Update $w_i'$ by adding the learning rate times the derivative of the error with respect to: $w_i : w_i' \leftarrow w_i - \alpha \frac{\partial E}{\partial w_i}$. That is equal to: $w_i' \leftarrow w_i - \alpha (\hat{y} - y) x_i$.
    \item Update $b : b' \leftarrow b - \alpha \frac{\partial E}{\partial w_i}$. That is equal to: $b' \leftarrow b - \alpha (\hat{y} - y)$.
  \end{itemize}
  \item Repeat step 3 until convergence.
\end{enumerate}

This makes the gradient descent algorithm very similar to the perceptron algorithm. The only difference is that gradient descent produces continuous probabilities for each point, while the perceptron classifies them as 0 or 1 (for the two label case).
